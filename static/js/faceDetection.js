...
export function isLivenessDetectionSupported() {
    try {
...
export var FacePresenceStatus;
(function (FacePresenceStatus) {
    FacePresenceStatus[FacePresenceStatus["NOT_FOUND"] = 0] = "NOT_FOUND";
    FacePresenceStatus[FacePresenceStatus["FOUND"] = 1] = "FOUND";
...
export function livenessDetectionSession(settings, faceDetectionCallback, faceCaptureCallback) {
    ...
    return liveFaceCapture(video, settings).pipe(map((capture) => {
...
export function liveFaceCapture(video, settings) {
    if (!settings) {
        settings = new FaceCaptureSettings();
    }
