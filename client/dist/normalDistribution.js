...
class Gamma {
    static regularizedGammaP(a, x, epsilon, maxIterations) {
    ...
    static regularizedGammaQ(a, x, epsilon, maxIterations) {
        if (isNaN(a) || isNaN(x) || a <= 0.0 || x < 0.0) {
            return NaN;
        }
        if (x == 0.0) {
        ...
        const cf = new ContinuedFraction((n, x1) => {
...
Gamma.halfLog2Pi = 0.5 * Math.log(2.0 * Math.PI);
class ContinuedFraction {
    constructor(getA, getB) {
        this.getA = getA;
        this.getB = getB;
    }
