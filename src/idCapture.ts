...
import { RecognizableFace, createRecognizableFace } from "./faceRecognition.js"

export async function detectIdCard(images: { front?: string, back?: string }): Promise<Array<{ face?: RecognizableFace }>> {
    let response = await fetch("/detectIdCard", {
        "method": "POST",
        "mode": "cors",
        "cache": "no-cache",
        "headers": {
    ...
    if (imageSide) {
        json.face = await createRecognizableFace(imageSide.fullDocumentImageBase64)
    }
    return json
}
...
export function captureIdCard(settings: IDCaptureSettings): Observable {
